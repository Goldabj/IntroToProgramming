...
Authors: Mark Hays, David Mutchler, Michael Wollowski, their colleagues,
         and SOLUTION by David Mutchler.  November 2014.
         Brendan Goldacker
...
def concentric_circles(window, outer_circle, n):
    ...
    # ------------------------------------------------------------------
    if n > 1:
        change_in_radius = (outer_circle.radius - 5) / (n - 1)
    else:
        change_in_radius = 0
    for k in range(n):
        new_circle = rg.Circle(outer_circle.center, outer_circle.radius - (k * change_in_radius))
...
def problem2(window, n, square):
    ...
    outer_circle = rg.Circle(square.center, square.length_of_each_side / 2)
    original_x = outer_circle.center.x
    number_of_circles = n
    for j in range(1, n + 1):
        outer_circle.center.x = original_x
        for k in range(n):
            concentric_circles(window, outer_circle, number_of_circles)
            outer_circle.center.x = outer_circle.center.x + 2 * outer_circle.radius
        number_of_circles = n - j
        outer_circle.center.y = outer_circle.center.y + 2 * outer_circle.radius
