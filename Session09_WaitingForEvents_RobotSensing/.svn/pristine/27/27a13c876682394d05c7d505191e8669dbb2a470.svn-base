...
def test_random_walk():
    ...
    circle1 = rg.Circle(rg.Point(100, 400), 60)
    circle1.fill_color = 'Blue'
    circle2 = rg.Circle(rg.Point(300, 400), 20)
    circle2.fill_color = 'green'
    random_walk(window1, circle1, .4, 1)
    print(random_walk(window1, circle1, .9, 100))
    random_walk(window1, circle2, .5, 10)
    print(random_walk(window1, circle2, .5, 40))
...
def random_walk(window, circle, probability_up, pixels_to_move):
    ...
    while True:
        ...
        if number < probability_up * 100:
            ...
            circle.move_center_to(x, y)
            window.render(0.01)
        if y > window.height - circle.radius or y < circle.radius:
            break
