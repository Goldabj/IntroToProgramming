...
def test_go_until_black():
    ...
    # DONE: Implement this WITH YOUR INSTRUCTOR.
    port = 'sim'
    robot = new_create.Create(port)

    reading_when_stopped = go_until_black(robot, 100)
    print(reading_when_stopped)
...
def go_until_black(robot, threshold):
    ...
    Returns the value of the sensor when the waiting stopped.
    OPTIONALLY, prints the sensor readings during the run.
    ...
    sensor_name = new_create.Sensors.cliff_front_left
...
def test_go_distance(function_to_test):
    ...
    # DONE: Do your testing in the simulator.
...
def go_distance_until_time(robot, centimeters, centimeters_per_second):
    ...
    distanceSensor = new_create.Sensors.distance

    robot.driveDirect(centimeters_per_second, centimeters_per_second)
    time.sleep(seconds)
    robot.stop()

    distance = robot.getSensor(distanceSensor)
...
def go_distance_until_sensor(robot, centimeters, centimeters_per_second):
    ...
    while True:
        distance = robot.getSensor(distanceSensor)
        totaldistance = totaldistance + distance / 10
        if totaldistance >= centimeters:
            break
        time.sleep(.01)
    robot.stop()
