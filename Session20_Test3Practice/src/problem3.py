...
def hourglass(window, n, point, radius, color):
    ...
    # ------------------------------------------------------------------
    hourglass_half(window, n, point, radius, color, 1)
    hourglass_half(window, n, point, radius, color, -1)


def hourglass_half(window, n, point, radius, color, direction):
    x = point.x
    y = point.y
    for j in range(n):
        center = rg.Point(x, y)
        for _ in range(j + 1):
            ...
            left = rg.Point(circle.center.x - radius, circle.center.y)
            right = rg.Point(circle.center.x + radius, circle.center.y)
        ...
        x = x - radius
        y = y + (math.sqrt(3) * radius) * direction
        center = rg.Point(x, y)
...
def many_hourglasses(window, square, m, colors):
    ...
    for k in range(m):
        rectangle = rg.Rectangle(point, width, height)
        rectangle.attach_to(window.initial_canvas)

        hourglass(window, k + 1, point, radius, colors[k % len(colors)])

        new_width = width + (radius * 2)
        height = height + (2 * radius * math.sqrt(3))
        point = rg.Point(point.x + (width / 2) + (new_width / 2),
                         point.y)
        width = new_width
